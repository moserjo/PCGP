...
class PCGP_Kernel(gpytorch.kernels.Kernel):
    def __init__(self, input_parameters, number_of_input_dimensions=1, num_tasks=4, **kwargs):
        super().__init__()
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        k01 = torch.zeros_like(xx[0], device=x1.device)
        k02 = A*(g*l**2 - l1*(l - (xx[0] - yy[0])**2))*torch.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
        k03 = torch.zeros_like(xx[0], device=x1.device)
        k10 = torch.zeros_like(xx[0], device=x1.device)
        k11 = A*torch.exp(-1/2*(xx[0] - yy[0])**2/l)
        k12 = torch.zeros_like(xx[0], device=x1.device)
        k13 = A*(g*l**2 - l2*(l - (xx[0] - yy[0])**2))*torch.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
        k20 = A*(g*l**2 - l1*(l - (xx[0] - yy[0])**2))*torch.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
        k21 = torch.zeros_like(xx[0], device=x1.device)
        k22 = A*(g**2*l**4 - 2*g*l**2*l1*(l - (xx[0] - yy[0])**2) + l1**2*(2*l**2 - 4*l*(xx[0] - yy[0])**2 + (l - (xx[0] - yy[0])**2)**2))*torch.exp(-1/2*(xx[0] - yy[0])**2/l)/l**4
        k23 = torch.zeros_like(xx[0], device=x1.device)
        k30 = torch.zeros_like(xx[0], device=x1.device)
        k31 = A*(g*l**2 - l2*(l - (xx[0] - yy[0])**2))*torch.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
        k32 = torch.zeros_like(xx[0], device=x1.device)
        k33 = A*(g**2*l**4 - 2*g*l**2*l2*(l - (xx[0] - yy[0])**2) + l2**2*(2*l**2 - 4*l*(xx[0] - yy[0])**2 + (l - (xx[0] - yy[0])**2)**2))*torch.exp(-1/2*(xx[0] - yy[0])**2/l)/l**4
        cov_m = torch.squeeze(torch.cat([
    torch.cat([k00, k01, k02, k03], dim=-1),
    torch.cat([k10, k11, k12, k13], dim=-1),
    torch.cat([k20, k21, k22, k23], dim=-1),
    torch.cat([k30, k31, k32, k33], dim=-1)], dim=-2))
        cov_f = rearrange(cov_m, "(t1 w1) (t2 w2)-> (w1 t1) (w2 t2)", t1=4, t2=4)
        return torch.diag(cov_f) if diag else cov_f


class PCGP_Model(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood,  model_parameters, number_of_input_dimensions = 1, num_tasks = 4, priors = None):
