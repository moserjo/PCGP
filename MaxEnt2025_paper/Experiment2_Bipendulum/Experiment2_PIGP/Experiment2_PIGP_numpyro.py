...
def kernel(x1, x2, parameters, include_noise=True, sigma = 5e-2): 
    ...
    k01 = jnp.zeros_like(xx[0])
    k02 = A*(g*l**2 - l1*(l - (xx[0] - yy[0])**2))*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
    k03 = jnp.zeros_like(xx[0])
    k10 = jnp.zeros_like(xx[0])
    k11 = A*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)
    k12 = jnp.zeros_like(xx[0])
    k13 = A*(g*l**2 - l2*(l - (xx[0] - yy[0])**2))*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
    k20 = A*(g*l**2 - l1*(l - (xx[0] - yy[0])**2))*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
    k21 = jnp.zeros_like(xx[0])
    k22 = A*(g**2*l**4 - 2*g*l**2*l1*(l - (xx[0] - yy[0])**2) + l1**2*(2*l**2 - 4*l*(xx[0] - yy[0])**2 + (l - (xx[0] - yy[0])**2)**2))*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)/l**4
    k23 = jnp.zeros_like(xx[0])
    k30 = jnp.zeros_like(xx[0])
    k31 = A*(g*l**2 - l2*(l - (xx[0] - yy[0])**2))*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)/l**2
    k32 = jnp.zeros_like(xx[0])
    k33 = A*(g**2*l**4 - 2*g*l**2*l2*(l - (xx[0] - yy[0])**2) + l2**2*(2*l**2 - 4*l*(xx[0] - yy[0])**2 + (l - (xx[0] - yy[0])**2)**2))*jnp.exp(-1/2*(xx[0] - yy[0])**2/l)/l**4
    k = jnp.squeeze(jnp.concatenate([
    jnp.concatenate([k00, k01, k02, k03], axis = -1),
    jnp.concatenate([k10, k11, k12, k13], axis = -1),
    jnp.concatenate([k20, k21, k22, k23], axis = -1),
    jnp.concatenate([k30, k31, k32, k33], axis = -1)], axis=-2))
